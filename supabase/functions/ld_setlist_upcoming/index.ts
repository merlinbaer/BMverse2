...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    return new Response(
      'Error in JOB:' + job.name + ' - Data load upcoming failed.',
      { status: 500 },
    )
  }
  ...
  try {
    ...
    job.log(
      '# upcoming concerts to be deleted because they are to old: ' +
        oldUpcomingConcerts.length,
    )
    ...
    job.error(
      `Error removing old upcoming concerts: ${JSON.stringify(err, null, 2)} `,
    )
    return new Response(
      'Error in JOB:' + job.name + ' - Removing old upcoming concerts',
      { status: 500 },
    )
  }
  ...
  try {
    ...
    for (const item of setlistArray) {
      const sourceRow = setlistUpcoming.find(
        row => row.setlist_id === item.setlist_id,
      )
      if (sourceRow) {
    ...
    const inserted_Rows = await insertUntypedData(
      setlistArray,
      'ld_setlist_upcoming',
      job.db,
    )
    ...
    job.error(
      `Error fetching API data: { fetched_setlists: ${setlistArray.length}; ${JSON.stringify(err, null, 2)} `,
    )
    return new Response('Error in JOB:' + job.name, { status: 500 })
  }

  await job.end(
    `total upcoming setlists: ${setlistArray.length}; deleted old upcoming setlists: ${deletedUpcomingConcerts};`,
  )
