...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    return new Response(
      'Error in JOB:' + job.name + ' - Videos loading failed',
      { status: 500 },
    )
  }
  ...
  if (youtube.length < 30) {
    ...
    return new Response('Error in JOB:' + job.name + ' - Not enough videos', {
      status: 500,
    })
  }
  ...
  try {
    ...
    if (missingVideos.length > 0) {
      insertedVideos = await insertUntypedData(
        missingVideos,
        'bm_videos',
        job.db,
      )
      const insertNewsMessage =
        insertedVideos === 1
    ...
    return new Response(
      'Error in JOB:' + job.name + ' - Insert videos failed',
      { status: 500 },
    )
  }
  ...
  try {
    ...
    job.error(
      `Error: Only ${deletedVideos} videos soft deleted: ${JSON.stringify(err, null, 2)} `,
    )
    return new Response(
      'Error in JOB:' + job.name + ' - Delete videos failed.',
      { status: 500 },
    )
  }
  ...
  try {
    ...
    job.error(
      `Error: Only ${updatedVideos} videos updated: ${JSON.stringify(err, null, 2)}`,
    )
    return new Response('Error in JOB:' + job.name + ' - Video update failed', {
      status: 500,
    })
  }

  await job.end(
    `Videos inserted: ${insertedVideos}; Updated videos: ${updatedVideos}; Videos deleted: ${deletedVideos};`,
  )
