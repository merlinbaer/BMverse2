...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    job.log(
      'Fatal error in job initialization: ' + JSON.stringify(err, null, 2),
    )
  ...
  try {
    ...
    job.log(
      `#bm_event_concert: ${concert.length}; #ld_setlist: ${setlist.length};`,
    )
    job.log(
      `#bm_event_concert_songs: ${concertSongs.length}; #ld_setlist_songs: ${setlistSongs.length};`,
    )
  } catch (err) {
    job.error(
      `Error: Reading concerts and song data failed: ${JSON.stringify(err, null, 2)} `,
    )
    return new Response(
      'Error in JOB:' + job.name + ' - Concerts and songs loading failed',
      { status: 500 },
    )
  }
  ...
  if (setlist.length < 400) {
    ...
    return new Response(
      'Error in JOB:' + job.name + ' - Not enough setlist data',
      { status: 500 },
    )
  }
  if (setlistSongs.length < 4000) {
    ...
    return new Response(
      'Error in JOB:' + job.name + ' - Not enough song data',
      { status: 500 },
    )
  }
  ...
  try {
    ...
    if (missingSetlist.length > 0) {
      insertedConcerts = await insertUntypedData(
        missingSetlist,
        'bm_event_concert',
        job.db,
      )
    ...
    if (missingSongs.length > 0) {
      insertedSongs = await insertUntypedData(
        missingSongs,
    ...
    job.error(
      `Error: Concerts and song insert failed: ${JSON.stringify(err, null, 2)} `,
    )
    return new Response(
      'Error in JOB:' + job.name + ' - Insert concerts and songs failed',
      { status: 500 },
    )
  }
  ...
  try {
    ...
    if (missingSetlist.length > 0) {
      for (const row of missingSetlist) {
        row.deleted = true
        const updatedRow = await updateUntypedData(
          'bm_event_concert',
          row,
          'setlist_id',
          job.db,
    ...
    job.error(
      `Error: Only ${deletedConcerts} concert soft deleted: ${JSON.stringify(err, null, 2)} `,
    )
    return new Response(
      'Error in JOB:' + job.name + ' - Delete concert failed.',
      { status: 500 },
    )
  }
  ...
  try {
    ...
    job.error(
      `Error: Only ${deletedConcerts} song soft deleted: ${JSON.stringify(err, null, 2)} `,
    )
    return new Response('Error in JOB:' + job.name + ' - Delete song failed.', {
      status: 500,
    })
  }
  ...
  try {
    ...
    if (changedSetlist.length > 0) {
      for (const row of changedSetlist) {
        ...
        const deletedSongRows = await updateUntypedData(
          'bm_event_concert_songs',
          songRow,
          'setlist_id',
          job.db,
    ...
    job.error(
      `Error: Only ${updatedConcerts} concerts and ${updatedOldSongs} songs updated: ${JSON.stringify(err, null, 2)}`,
    )
    return new Response(
      'Error in JOB:' + job.name + ' - Concert update failed',
      { status: 500 },
    )
  }
  try {
    ...
    job.error(
      `Error: Reloading song data detailed: ${JSON.stringify(err, null, 2)} `,
    )
    return new Response(
      'Error in JOB:' + job.name + ' - Song reloading failed',
      { status: 500 },
    )
  }
  try {
    ...
    const missingSongs = missingBySetlistId(setlistSongs, concertSongs)
    if (missingSongs.length > 0) {
      updatedNewSongs = await insertUntypedData(
        missingSongs,
        'bm_event_concert_songs',
        job.db,
      )
      job.log(
        `#Old songs before update: ${updatedOldSongs}; #New songs after update: ${updatedNewSongs};`,
      )
    }
  } catch (err) {
    job.error(
      `Error: Insert of updated songs failed: ${JSON.stringify(err, null, 2)} `,
    )
    return new Response(
      'Error in JOB:' + job.name + ' - Insert updated songs failed',
      { status: 500 },
    )
  }
  ...
  try {
    ...
    job.log(
      `#bm_event_concert_upcoming: ${concert.length}; #ld_setlist_upcoming: ${setlist.length};`,
    )
  } catch (err) {
    job.error(
      `Error: Reading upcoming concerts data failed: ${JSON.stringify(err, null, 2)} `,
    )
    return new Response(
      'Error in JOB:' + job.name + ' - Upcoming concerts loading failed',
      { status: 500 },
    )
  }
  ...
  if (setlist.length < concert.length - 1) {
    job.log(
      `WARNING: ld table seems to have not enough rows. Check table ld concert upcoming.`,
    )
  }
  try {
    ...
    if (missingSetlist.length > 0) {
      insertedUpcomingConcerts = await insertUntypedData(
        missingSetlist,
    ...
    job.error(
      `Error: Upcoming concerts insert failed: ${JSON.stringify(err, null, 2)} `,
    )
    return new Response(
      'Error in JOB:' + job.name + ' - Insert upcoming concerts failed',
      { status: 500 },
    )
  }
  try {
    ...
    job.error(
      `Error: Only ${deletedUpcomingConcerts} upcoming concerts soft deleted: ${JSON.stringify(err, null, 2)} `,
    )
    return new Response(
      'Error in JOB:' + job.name + ' - Delete upcoming concerts failed.',
      { status: 500 },
    )
  }
  if (
