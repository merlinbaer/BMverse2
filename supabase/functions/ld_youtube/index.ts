...
Deno.serve(async (req: Request) => {
  ...
  try {
    const channelTable = await selectAllRows(
      'ld_video_channels',
      'channel_id',
      job.db,
    )
    const channelList = channelTable
      .map(channel => channel.channel_id)
      .join(',')
    const channelApi = await fetchYoutubeApi(
    ...
    const videoApi = await fetchYoutubeApi(
      'https://www.googleapis.com/youtube/v3/videos?part=id,contentDetails,snippet,statistics',
      videoList,
    )
    ...
    return new Response('Error in JOB:' + job.name + ' - Reading video input', {
      status: 500,
    })
  }
  ...
  try {
    ...
    return new Response('Error in JOB:' + job.name + ' - Insert video data', {
      status: 500,
    })
  }

  await job.end(
    `#Jobs: Deleted rows in ld_youtube ${deletedRows}; Inserted rows in ld_youtube ${inserted_Rows}`,
  )
...
function transformVideoData(
  ...
  for (const obj of fetchedArray) {
    ...
    video.channel_title = findObject(
      channelArray,
      'channel_id',
      obj.snippet.channelId,
    ).channel_title
    video.channel_decription = findObject(
      channelArray,
      'channel_id',
      obj.snippet.channelId,
    ).channel_decription
    video.channel_customurl = findObject(
      channelArray,
      'channel_id',
      obj.snippet.channelId,
    ).channel_customurl
    video.channel_type = findObject(tableArray, 'video_id', obj.id).channel_type
    video.channel_artwork = findObject(
      channelArray,
      'channel_id',
      obj.snippet.channelId,
    ).channel_artwork
    video.video_title_original = obj.snippet.title
    const title_edited = findObject(
      tableArray,
      'video_id',
      obj.id,
    ).video_title_edited
...
function findObject<T, K extends keyof T>(
  array: T[],
  key: K,
  value: T[K],
): T | undefined {
