...
Deno.serve(async (req: Request) => {
  ...
  // load mapping tables
  try {
    songsMapping = await selectAllRows(
      'ld_song_mapping',
      'song_name_original, song_title',
      job.db,
    )
    songsArray = await selectAllRows(
      'bm_songs',
      'song_title, song_preview_artwork',
      job.db,
    )
  } catch (err) {
    job.error(`Error reading mapping tables: ${JSON.stringify(err, null, 2)} `)
    return new Response('Error in JOB:' + job.name + ' - Data mapping', {
      status: 500,
    })
  }
  ...
  try {
    ...
    if (pages >= 24) {
      for (let i = 1; i <= pages; i++) {
        ...
        setlistArray = [
          ...setlistArray,
          ...transformToSetlist(fetchedData.setlist),
        ]
        setlistSongsArray = [
          ...setlistSongsArray,
          ...transformToSongs(fetchedData.setlist, songsMapping, songsArray),
        ]
      }
    } else {
    ...
    let inserted_Rows = await insertUntypedData(
      setlistArray,
      'ld_setlist',
      job.db,
    )
    job.log(`${inserted_Rows} rows inserted in table ls_setlist`)
    inserted_Rows = await insertUntypedData(
      setlistSongsArray,
      'ld_setlist_songs',
      job.db,
    )
    ...
    job.error(
      `Error fetching API data: { total pages: ${pages}; fetched_setlists: ${setlistArray.length}; fetched_songs: ${setlistSongsArray.length} }; ${JSON.stringify(err, null, 2)} `,
    )
    return new Response('Error in JOB:' + job.name, { status: 500 })
  }

  await job.end(
    `total_setlists: ${setlistArray.length}; total_songs: ${setlistSongsArray.length}`,
  )
...
async function fetchSetlist(page: number) {
  const url =
    'https://api.setlist.fm/rest/1.0/artist/27e2997f-f7a1-4353-bcc4-57b9274fa9a4/setlists?p=' +
    String(page)
...
function transformToSetlist(fetchedArray: any[]): any[] {
  ...
  for (let obj of fetchedArray) {
    ...
    obj.setlist_artwork =
      'https://flagsapi.com/' +
      obj.setlist_venue_city_country_code +
      '/shiny/64.png'
    ...
    for (const set of obj.sets.set) {
      ...
      for (const song of set.song) {
        ...
        if (!(tape && song.name !== 'Megitsune')) {
          ...
          const mappedSongObject = songsMapping.find(
            obj => obj['song_name_original'] === song.name,
          )
          const mappedSongName = mappedSongObject
            ? mappedSongObject.song_title
            : song.name
          const songObject = songsArray.find(
            obj => obj['song_title'] === mappedSongName,
          )
          const songArtwork = songObject
            ? songObject.song_preview_artwork
            : 'unknown'
          songsPageArray.push({
            setlist_id: obj.id,
